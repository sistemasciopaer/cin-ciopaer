...
export function Relatorios() {
 ...
 function gerarHTMLRelatorio({ grupos, dataLabel, publico }) {
   let tabelaHTML = ''
   grupos.forEach(g => {
     ...
     tabelaHTML += `
       <div class="grupo">
         <div class="grupo-header">
           ...
           <span class="grupo-vagas">${publico ? g.itens.length + ' servidor(es)' : ocupados + '/' + capacidade + ' vagas'}</span>
         </div>
         <table>
           <thead>
             <tr>
               ...
               <th>CPF</th>
               <th>Tipo</th>
               ${!publico ? '<th>Status</th><th>Responsável</th>' : ''}
             </tr>
           </thead>
           <tbody>
             ${g.itens.map((a, i) => `
               <tr class="${!publico && a.status === 'PRESENTE' ? 'presente' : ''}">
                 <td>${i + 1}</td>
                 <td>${a.nome_agendado}</td>
                 <td>${publico ? mascaraCPF(a.cpf_agendado) : formatarCPF(a.cpf_agendado)}</td>
...
<style>
 ...
 .aviso { background:#fff8ec; border:1px solid #E67E22; border-radius:6px; padding:8px 14px; margin-bottom:16px; font-size:10px; color:#b7610a; }
...
<div class="cabecalho">
 ...
 <div class="cabecalho-texto">
   <h1>CIOPAER</h1>
   <p>${publico ? 'Relação para Divulgação' : 'Relatório Interno'} — Emissão da CIN</p>
 </div>
</div>
${publico ? `<div class="aviso">
 ⚠️ CPFs parcialmente ocultados conforme a LGPD (Lei nº 13.709/2018).
 O documento completo com CPF íntegro está disponível apenas para uso interno no dia do atendimento.
</div>` : ''}
<div class="meta">
 <span><strong>Período:</strong> ${dataLabel}</span>
 <span><strong>Total:</strong> ${grupos.reduce((acc, g) => acc + g.itens.length, 0)} agendamentos</span>
...
<div class="rodape">
 Sistema de Agendamentos CIOPAER — ${publico ? 'Documento para divulgação — CPFs protegidos pela LGPD' : 'Documento interno — uso restrito'}
</div>
</body></html>`
 }
 ...
 function downloadPDF() {
   ...
   const html      = gerarHTMLRelatorio({ grupos, dataLabel, publico: false })
   baixarHTML(html, `relatorio_interno_${filtroData === 'TODOS' ? 'todos' : filtroData}.html`)
 }

 function downloadPDFPublico() {
   const lista     = agsFiltrados.filter(a => ['AGENDADO','PRESENTE'].includes(a.status))
   const grupos    = agrupadoPorDataHora(lista)
   const dataLabel = filtroData === 'TODOS' ? 'Todos os dias' : fmtData(filtroData)
   const html      = gerarHTMLRelatorio({ grupos, dataLabel, publico: true })
   baixarHTML(html, `relatorio_divulgacao_${filtroData === 'TODOS' ? 'todos' : filtroData}.html`)
 }
   ...
   <div style={{ minHeight:'100vh', background:'var(--bg)', padding:'0 0 60px' }}>
     ...
     <div style={{ padding:'0 16px', marginTop:-16 }}>
       ...
       <div style={{ ...card, padding:'12px 14px' }}>
         ...
         <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
           {[
             { label:'📄 CSV',               fn: downloadCSV,        title: 'Dados completos em planilha' },
             { label:'📋 Relatório interno',  fn: downloadPDF,        title: 'Com CPF completo — uso interno' },
             { label:'📤 Para divulgação',    fn: downloadPDFPublico, title: 'CPF mascarado — LGPD' },
           ].map(b => (
         ...
         <p style={{ color:'var(--texto-3)', fontSize:'0.7rem', marginTop:8, lineHeight:1.5 }}>
           Os relatórios são baixados como HTML. Abra e toque em <strong>Imprimir / Salvar PDF</strong>.
         </p>
       </div>
